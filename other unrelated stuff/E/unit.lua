          ...
          if is_u and sameFloat(unit, on) then
            if timecheck(unit,"be","xwx") and (timecheck(on,"be","u") or timecheck(on,"be","u too") or timecheck(on,"be","u tres")) then
              if settings["ads"] then
                local title = "XwX"
                local message = "Report this Error to Microbab?"
                local buttons = {"OK", "Get dat mf'er", escapebutton = 2}
    
                local pressedbutton = love.window.showMessageBox(title, message, buttons)
                if pressedbutton == 1 then
                    love.system.openURL("https://www.youtube.com/watch?v=5BZLz21ZS_Y")
                elseif pressedbutton == 2 then
                    love.system.openURL("https://www.youtube.com/watch?v=4QEqBaCi6gE")
                end
              end
              error(INVALID_VALUE)
            else
