...
function updateUnits(undoing, big_update)
  ...
  if (big_update and not undoing) then
    ...
    local issink2 = getUnitsWithEffect("sink")
    for _,unit in ipairs(issink2) do
      local stuff = getUnitsOnTile(unit.x, unit.y, {not_destroyed = true, thicc = thicc_units[unit]})
      for _,on in ipairs(stuff) do
        if unit ~= on and on.fullname ~= "no1" and sameFloat(unit, on) then
          local ignore_unit = ignoreCheck(unit, on)
          local ignore_on = ignoreCheck(on, unit, "sink")
          if ignore_unit or ignore_on then
            if timecheck(unit,"be","sink") and timecheck(on) then
              if ignore_unit then
                table.insert(to_destroy, unit)
              end
              if ignore_on then
    ...
    local isaw = getUnitsWithEffect("defeat")
    for _,unit in ipairs(isaw) do
      local stuff = getUnitsOnTile(unit.x, unit.y, {not_destroyed = true, checkmous = true, thicc = thicc_units[unit]})
      for _,on in ipairs(stuff) do
    ...
    local isnedkee = getUnitsWithEffect("shut")
    for _,unit in ipairs(isnedkee) do
      local stuff = getUnitsOnTile(unit.x, unit.y, {not_destroyed = true, checkmous = true, thicc = thicc_units[unit]})
      for _,on in ipairs(stuff) do
        if hasProperty(on, "open") and sameFloat(unit, on) then
          local ignore_unit = ignoreCheck(unit, on, "open")
          local ignore_on = ignoreCheck(on, unit, "shut")
          if ignore_unit or ignore_on then
            if timecheck(unit,"be","shyt") and timecheck(on,"be","open") then
              if ignore_unit then
                table.insert(to_destroy, unit)
              end
              if ignore_on then
...
function levelBlock()
 ...
 local gaemsafe = hasProperty("gaem","protecc")
  ...
  if hasProperty("gaem","stalkredge") then
    x, y, displayindex = love.window.getPosition()
    love.window.setPosition(x+64, y, displayindex )
  end
  
  if hasProperty("gaem","stalkskye") then
    x, y, displayindex = love.window.getPosition()
    love.window.setPosition(x, y-64, displayindex )
  end
  
  if hasProperty("gaem","stalkflor") then
    x, y, displayindex = love.window.getPosition()
    love.window.setPosition(x, y+64, displayindex )
  end
  
  if hasProperty("gaem","haetskye") then
    x, y, displayindex = love.window.getPosition()
    love.window.setPosition(x, y+10000, displayindex )
  end
  
  if hasProperty("gaem","haetflor") then
    x, y, displayindex = love.window.getPosition()
    love.window.setPosition(x, y-10000, displayindex )
  end
